...
def get_beers_by_brewery(_brewery, order_by="rating"):
    query = Beers.query.filter_by(brewery=_brewery).order_by(
        getattr(Beers, order_by).desc()
    )
    return query.all()
...
def get_beers_by_year(_year, order_by="drink_datetime"):
    return (
        Beers.query.filter(extract("year", Beers.drink_datetime) == _year)
...
def get_beers_by_style(_style, order_by="rating"):
    return (
        Beers.query.filter_by(style=_style)
...
def get_beers_by_rating(_rating, order_by="brewery"):
    return (
        Beers.query.filter_by(rating=_rating)
        .order_by(getattr(Beers, order_by).asc())
        .all()
    )
...
def get_beers_by_attribute(_attribute, _value, order_by="rating"):
    return (
        Beers.query.filter(getattr(Beers, _attribute) == _value)
        .order_by(getattr(Beers, order_by).desc())
        .all()
    )
