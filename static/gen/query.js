$(document).ready(function(){ var csrftoken=$('meta[name=csrf-token]').attr('content');var queryResults=Handlebars.templates['query-results'];$.ajaxSetup({beforeSend:function(xhr,settings){if(!/^(GET|HEAD|OPTIONS|TRACE)$/i.test(settings.type)&&!this.crossDomain){xhr.setRequestHeader("X-CSRFToken",csrftoken)}}});var hideDetails=function(){$('#results-column').removeClass('mobile-results');$('#detail-column').removeClass('mobile-detail').hide();$('.search').show();history.pushState('',document.title,window.location.pathname);}
var detailify=function(evt){var detail=$('#beer-detail'),name=$(this).find('.name').text(),brewery=$(this).find('.brewery').text(),country=$(this).find('.country').text(),style=$(this).find('.style').text(),rating=$(this).attr('data-rating'),rating_txt=$(this).attr('data-rating-text'),abv=$(this).attr('data-abv'),d_city=$(this).attr('data-city'),d_country=$(this).attr('data-country'),d_month=$(this).attr('data-month'),d_year=$(this).attr('data-year'),beer_with=$(this).attr('data-beerwith'),beer_year=$(this).attr('data-beeryear'),tags=[]
$('#start').hide(); detail.find('.rating-text').removeAttr("class").addClass("rating-text");detail.find('.name').text(name);detail.find('.brewery').text(brewery);detail.find('.country').text(country);detail.find('.style').text(style);detail.find('.name').text(name);detail.find('.abv').text(abv);detail.find('.rating').text(rating);detail.find('.rating-text').addClass(rating_txt);detail.find('.drink-city').text(d_city);detail.find('.drink-country').text(d_country);detail.find('.drink-month').text(d_month);detail.find('.drink-year').text(d_year);tag_node=detail.find('.tags');if(tags.length>0){for(tag in tags){tag_node.append("<span class='tag'>"+tag+"</span>");}
tag_node.show();}else{tag_node.hide();}
beeryear_node=detail.find('.beer-year');if(beer_year){beeryear_node.text(beer_year);beeryear_node.show();}else{beeryear_node.hide();}
beerwith_node=detail.find('.beer-with');if(beer_with){beerwith_node.text(beer_with);beerwith_node.show();}else{beerwith_node.hide();} 
if($('#mobilizer').css('display')==='block'){$('#detail-column').show().addClass('mobile-detail');$('#results-column').addClass('mobile-results');$('#detail-back').show();$('.search').hide();window.location.hash=new Date().getTime();window.onhashchange=function(){if(!location.hash){hideDetails();}}}}
...
console.log(results)
$('#query-results').html(queryResults(results));$('#results-count').text('Beers: '+results.results.length);$('#search-results-header').text('Search Results');$('.result').click(detailify);}});}
$('#query').on("input",$.debounce(query,500));$('#detail-back').click(hideDetails);});(function(){var template=Handlebars.template,templates=Handlebars.templates=Handlebars.templates||{};templates['query-results']=template({"1":function(depth0,helpers,partials,data){var helper,functionType="function",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression;return"  <div class=\"result\" data-rating=\""
