...
if($('#mobilizer').css('display')==='block'){$('#detail-column').show().addClass('mobile-detail');$('#results-column').addClass('mobile-results');$('#detail-back').show();}}
...
console.log(results)
$('#query-results').html(queryResults(results));$('#results-count').text('Beers: '+results.results.length);$('.result').click(detailify);}});}
$('#query').on("input",$.debounce(query,500));$('#detail-back').click(function(){$('#results-column').removeClass('mobile-results');$('#detail-column').removeClass('mobile-detail').hide();});});(function(){var template=Handlebars.template,templates=Handlebars.templates=Handlebars.templates||{};templates['query-results']=template({"1":function(depth0,helpers,partials,data){var helper,functionType="function",helperMissing=helpers.helperMissing,escapeExpression=this.escapeExpression;return"  <div class=\"result\" data-rating=\""
